...
# ───────────────────── 100% RFC 8439 ChaCha20-Poly1305 ─────────────────────
...
# ← FIXED: renamed to 'alphabet' so it never shadows secrets.choice
def genpw():
    alphabet = [c for c in string.ascii_letters + string.digits + "!@#$%^&*_-+=" if c not in "lI1O0|"]
    return "".join(choice(alphabet) for _ in range(22))
...
print("KAPTANOVI iSH — FINAL — NO MORE BUGS — NOV 17 2025")
while True:
    print("\n=== KAPTANOVI SAFE ===")
    if not V.exists():
